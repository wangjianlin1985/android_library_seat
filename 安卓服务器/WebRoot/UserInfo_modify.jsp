...
function checkForm() {
    ...
    var blackFlag = document.getElementById("userInfo.blackFlag").value;
    if(blackFlag=="") {
        alert('请输入是否黑名单!');
        return false;
    }
...
<table width='100%' cellspacing='1' cellpadding='3' class="tablewidth">
  ...
  <tr>
    <td width=30%>用户类型:</td>
    <td width=70%>
      <select name="userInfo.userTypeObj.userTypeId">
      <%
        for(UserType userType:userTypeList) {
          String selected = "";
          if(userType.getUserTypeId() == userInfo.getUserTypeObj().getUserTypeId())
            selected = "selected";
      %>
          <option value='<%=userType.getUserTypeId() %>' <%=selected %>><%=userType.getUserTypeName() %></option>
      <%
        }
      %>
    </td>
  ...
  <tr>
    <td width=30%>所在班级:</td>
    <td width=70%>
      <select name="userInfo.classObj.classNo">
      <%
        for(ClassInfo classInfo:classInfoList) {
          String selected = "";
          if(classInfo.getClassNo().equals(userInfo.getClassObj().getClassNo()))
            selected = "selected";
      %>
  ...
  <tr>
    ...
    <td width=70%><input id="userInfo.blackFlag" name="userInfo.blackFlag" type="text" size="20" value='<%=userInfo.getBlackFlag() %>'/></td>
  </tr>

  <tr>
    <td width=30%>信用分:</td>
    <td width=70%><input id="userInfo.creditScore" name="userInfo.creditScore" type="text" size="8" value='<%=userInfo.getCreditScore() %>'/></td>
  </tr>
