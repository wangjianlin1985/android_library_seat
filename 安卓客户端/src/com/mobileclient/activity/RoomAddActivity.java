			...
			public void onClick(View v) {
				try {
					...
					if(ET_roomName.getText().toString().equals("")) {
						Toast.makeText(RoomAddActivity.this, "阅览室名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(room.getRoomPhoto() != null) {
						...
						RoomAddActivity.this.setTitle("正在上传图片，稍等...");
						String roomPhoto = HttpUtil.uploadFile(room.getRoomPhoto());
						RoomAddActivity.this.setTitle("图片上传完毕！");
					...
					if(ET_roomPlace.getText().toString().equals("")) {
						Toast.makeText(RoomAddActivity.this, "阅览室位置输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_seatNum.getText().toString().equals("")) {
						Toast.makeText(RoomAddActivity.this, "总座位数输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传阅览室信息*/
					RoomAddActivity.this.setTitle("正在上传阅览室信息，稍等...");
