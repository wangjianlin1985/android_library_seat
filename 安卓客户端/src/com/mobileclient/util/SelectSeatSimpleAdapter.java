...
/*需要绑定的数据*/
  ...
  public View getView(int position, View convertView, ViewGroup parent) { 
	  ...
	  /*设置各个控件的展示内容*/
	  holder.tv_selectId.setText("选座id：" + mData.get(position).get("selectId").toString());
	  holder.tv_seatObj.setText("座位编号：" + (new SeatService()).GetSeat(Integer.parseInt(mData.get(position).get("seatObj").toString())).getSeatCode());
	  holder.tv_userObj.setText("选座用户：" + (new UserInfoService()).GetUserInfo(mData.get(position).get("userObj").toString()).getName());
	  holder.tv_startTime.setText("选座开始时间：" + mData.get(position).get("startTime").toString());
	  holder.tv_endTime.setText("选座结束时间：" + mData.get(position).get("endTime").toString());
	  holder.tv_seatState.setText("选座状态：" + mData.get(position).get("seatState").toString());
	  /*返回修改好的view*/
	  return convertView; 
    } 
