...
/*需要绑定的数据*/
  ...
  public View getView(int position, View convertView, ViewGroup parent) { 
	  ...
	  /*设置各个控件的展示内容*/
	  holder.tv_jcId.setText("奖惩id：" + mData.get(position).get("jcId").toString());
	  holder.tv_jcType.setText("奖惩类型：" + mData.get(position).get("jcType").toString());
	  holder.tv_title.setText("奖惩标题：" + mData.get(position).get("title").toString());
	  holder.tv_userObj.setText("奖惩用户：" + (new UserInfoService()).GetUserInfo(mData.get(position).get("userObj").toString()).getName());
	  holder.tv_creditScore.setText("信用分数：" + mData.get(position).get("creditScore").toString());
	  holder.tv_jcTime.setText("奖惩时间：" + mData.get(position).get("jcTime").toString());
	  /*返回修改好的view*/
	  return convertView; 
    } 
