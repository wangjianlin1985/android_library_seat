...
public class UserInfoService {
	...
	public String AddUserInfo(UserInfo userInfo) {
		...
		params.put("userTypeObj", userInfo.getUserTypeObj() + "");
		params.put("classObj", userInfo.getClassObj());
		...
		params.put("blackFlag", userInfo.getBlackFlag());
		params.put("creditScore", userInfo.getCreditScore() + "");
	...
	public List<UserInfo> QueryUserInfo(UserInfo queryConditionUserInfo) throws Exception {
		...
		if(queryConditionUserInfo != null) {
			...
			urlString += "&userTypeObj=" + queryConditionUserInfo.getUserTypeObj();
			urlString += "&classObj=" + URLEncoder.encode(queryConditionUserInfo.getClassObj(), "UTF-8") + "";
			...
			urlString += "&blackFlag=" + URLEncoder.encode(queryConditionUserInfo.getBlackFlag(), "UTF-8") + "";
		}
		...
		try {
			...
			for (int i = 0; i < length; i++) {
				...
				userInfo.setUserTypeObj(object.getInt("userTypeObj"));
				userInfo.setClassObj(object.getString("classObj"));
				...
				userInfo.setBlackFlag(object.getString("blackFlag"));
				userInfo.setCreditScore((float) object.getDouble("creditScore"));
